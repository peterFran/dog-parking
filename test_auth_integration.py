...
def get_firebase_token():
    ...
    if not token:
        ...
        print("Set FIREBASE_TEST_TOKEN environment variable with a valid Firebase ID token")
...
def test_owner_registration():
    ...
    if response and response.status_code == 201:
        ...
        print(f"❌ Owner registration failed: {response.status_code if response else 'No response'}")
        if response:
...
def test_owner_profile():
    ...
    if response and response.status_code == 200:
        ...
        print(f"❌ Owner profile retrieval failed: {response.status_code if response else 'No response'}")
        if response:
            print(f"Response: {response.text}")
        return None
...
def test_dog_registration():
    ...
    if response and response.status_code == 201:
        ...
        print(f"❌ Dog registration failed: {response.status_code if response else 'No response'}")
        if response:
            print(f"Response: {response.text}")
        return None
...
def test_dog_list():
    ...
    if response and response.status_code == 200:
        ...
        print(f"❌ Dog list retrieval failed: {response.status_code if response else 'No response'}")
        if response:
            print(f"Response: {response.text}")
        return None
...
def test_venue_list():
    ...
    if response and response.status_code == 200:
        ...
        print(f"❌ Venue list retrieval failed: {response.status_code if response else 'No response'}")
        if response:
            print(f"Response: {response.text}")
        return None
...
def main():
    ...
    if (results['dog_registration'] and 
        results['venue_list'] and 
        results['venue_list'].get('venues')):
